...
module Main where
  ...
  import Text.PrettyPrint.HughesPJ (render,text,(<>),Doc,parens)
...
---------------------
  ...
  data State = S { inter :: Bool,       -- True, si estamos en modo interactivo.
                   ...
                   ve :: NameEnv Value Type  -- Entorno con variables globales y su valor
                 }
...
--        state' <- compileFile (state {lfile=prelude, inter=False}) prelude
  ...
  handleCommand state@(S {..}) cmd
    =  case cmd of
         ...
         FindType s -> do x' <- parseIO "<interactive>" term s
                          t <- case x' of
                                 Nothing -> return $ Left "Error en el parsing."
                                 Just x -> return $ infer ve $ conversion $ x
                          case t of
                            Left err -> putStrLn ("Error de tipos: " ++ err) >> return ()
  ...
  parseIO f p x = case parse (whiteSpace simplytyped >> p >>= \ y -> eof >> return y) f x of
  ...
  handleStmt state stmt =
    do
      case stmt of
          Let x e    -> checkType x e
          Eval e     -> checkType it e
    where
      checkType i t = do
