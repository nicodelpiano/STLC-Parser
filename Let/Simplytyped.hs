...
-----------------------
  ...
  printTerm' b bs fs (Bound j) = text $ bs !! j
  printTerm' b bs fs (Free (Global n)) = text n
  printTerm' i bs fs (t :@: u) = parenIf (i < 1) $ printTerm' 2 bs fs t <+> printTerm' i bs fs u
  printTerm' i bs (f:fs) (Lam t u) = parenIf (i > 1) $ text "\\" <> text f <> text ":" <> printType t <> text "." <> printTerm' 1 (f:bs) fs u
  printTerm' i bs (f:fs) (TLet t u) = parenIf (i > 1) $ text "let " <> text f <> text " = " <> printTerm' i (f:bs) fs t <> text " in " <> printTerm' 1 (f:bs) fs u
...
-----------------------
  ...
  eval e (Lam t u :@: Lam s v) = eval e (sub 0 (Lam s v) u)
